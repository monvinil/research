#!/usr/bin/env python3
"""
Merge v3-8 model cards from Healthcare + Retail deep dives
into the existing v3-7 final inventory (389 models).

Input files:
  1. v3-7_normalized_final_2026-02-12.json        (389 models, the base)
  2. v3-8_healthcare_deep_dive_2026-02-12.json     (12 new Healthcare cards)
  3. v3-8_retail_deep_dive_2026-02-12.json         (12 new Retail cards)

Produces:
  - v3-8_normalized_2026-02-12.json  (full inventory)
...
EXISTING_FILE = BASE / "v3-7_normalized_final_2026-02-12.json"
OUTPUT_FILE = BASE / "v3-8_normalized_2026-02-12.json"
UI_OUTPUT = UI_DIR / "models.json"

NEW_CARD_FILES = [
    BASE / "v3-8_healthcare_deep_dive_2026-02-12.json",
    BASE / "v3-8_retail_deep_dive_2026-02-12.json",
]
...
def normalize_new_card(card):
    ...
    model = {
        ...
        "key_v3_context": None,
        "source_batch": card.get("source_batch", "v38_deep_dive"),
...
def main():
    print("=" * 70)
    print("MERGE v3-8 CARDS: Healthcare + Retail deep dives into 389-model base")
    print("=" * 70)
    print()
    ...
    for card_file in NEW_CARD_FILES:
        ...
        if not file_source_batch:
            ...
            if "healthcare" in stem:
                file_source_batch = "v38_deep_dive_healthcare"
            elif "retail" in stem:
                file_source_batch = "v38_deep_dive_retail"
            else:
                file_source_batch = "v38_deep_dive"
        for c in cards:
    ...
    output = {
        ...
        "engine_version": "3.8",
        "description": (
            f"v3-8 'Close the Loop' cycle: {len(existing_models)} base + "
            f"{new_count} new model cards from Healthcare and Retail Trade deep dives. "
            f"First retail coverage (15.5M workers). Healthcare expanded to 20 total cards. "
            f"All {len(all_models)} models sorted by composite, re-ranked. "
            f"Also produced: 53 signals, 9 counter-signals, P-040 Inverse Baumol cross-sector test."
        ),
        "rating_system": existing_data.get("rating_system", {}),
        "summary": {
            "total_models": len(all_models),
            "existing_models": len(existing_models),
            "new_v38_cards": new_count,
    ...
    for m in all_models[:20]:
        flag = " [NEW]" if m.get("source_batch", "").startswith("v38_") else ""
    ...
    print()
    print("  New v3-8 cards placement:")
    for m in all_models:
        if m.get("source_batch", "").startswith("v38_"):
            print(f"    {m['rank']:3d}. {m['composite']:6.2f}  "
